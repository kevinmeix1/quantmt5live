...
4. **Go live small.** Edit `run_live.bat` to a tiny `--max-order-lots` (e.g.
   0.05). After the tiny manual/live confirmation, the staged competition cap
   used by `run_live.bat` is `0.25` lots per order. Then:
   ```powershell
   .\run_live.bat
   ```
...
quanthack live-trade --config configs\competition.toml --adapter mt5 ^
  --poll-seconds 60 --iterations 100000 --max-order-lots 0.25 ^
