...
# O(1) lookup table built once at import; DEFAULT_INSTRUMENTS is immutable.
_BY_SYMBOL: dict[str, InstrumentSpec] = {
    instrument.symbol: instrument for instrument in DEFAULT_INSTRUMENTS
}
...
def instrument_for(symbol: str) -> InstrumentSpec:
    instrument = _BY_SYMBOL.get(normalize_symbol(symbol))
    if instrument is not None:
        return instrument
    valid = ", ".join(spec.display_symbol for spec in DEFAULT_INSTRUMENTS)
