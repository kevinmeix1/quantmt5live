...
class AppConfig:
    ...
    def validate_symbols(self) -> None:
        """Validate every strategy config's ``symbol`` against the instrument registry.

        Opt-in (not called by ``load_config``) so it never breaks an otherwise
        usable config; intended for preflight/startup to surface typos early
        rather than at first trade. Raises ``KeyError`` for an unknown symbol.
        """
