...
def _recent_valid_prices(prices: Sequence[float], lookback: int) -> list[float] | None:
    ...
    # Slice first, then copy: the backtest passes the full growing price history
    # each bar, so list(prices)[-lookback:] would copy the entire history (O(N))
    # just to keep the last `lookback` items. Slicing first is O(lookback) with
    # identical contents.
