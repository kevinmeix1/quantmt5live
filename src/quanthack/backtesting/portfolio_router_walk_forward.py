...
class PortfolioRouterWalkForwardFold:
    ...
    def selected_was_test_best(self) -> bool:
        return self.selected_weights == self.test_best_candidate.weights
    ...
    for fold_index, start in enumerate(
        ...
        folds.append(
            PortfolioRouterWalkForwardFold(
                ...
                selected_train_candidate=train_result.best,
...
def _matching_candidate(
    ...
    for candidate in result.candidates:
        if candidate.weights == weights:
            return candidate
    raise ValueError(f"selected router weights not found in test window: {weights.label}")
